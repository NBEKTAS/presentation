...
export const Slide5NestedNexus: React.FC = () => {
  ...
  return (
    <div className="w-full h-full min-h-0 flex flex-col justify-between max-w-[100rem] mx-auto overflow-hidden">
      ...
      <div className="flex-1 min-h-0 w-full bg-white rounded-2xl border border-slate-200 p-4 shadow-sm grid grid-cols-12 gap-4 items-center overflow-hidden">
        {/* Left Column: Concentric Circles Diagram (5 cols) */}
        <div className="col-span-5 h-full flex flex-col items-center justify-center p-2">
          <svg viewBox="0 0 520 480" className="w-full h-full max-h-[540px] overflow-visible">
        ...
        <div className="col-span-7 h-full flex flex-col justify-between gap-2 py-0.5 min-h-0">
          {tiers.map((tier) => (
            <div
              key={tier.id}
              className={`flex-1 min-h-0 px-4 py-2 rounded-xl border ${tier.color} shadow-2xs flex flex-col justify-center`}
            >
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-2">
                  <tier.icon className="w-5 h-5 text-slate-800 shrink-0" />
                  <h3 className="font-bold text-xl text-slate-900 truncate">
                    {tier.name}
                  </h3>
                </div>
                <span className={`text-base px-2 py-0.5 rounded-full font-bold whitespace-nowrap overflow-hidden text-ellipsis ml-2 ${tier.tagColor}`}>
                  {tier.role}
                </span>
              </div>
              <ul className="space-y-0.5 text-lg text-slate-700 min-h-0">
